...
use Wengg\WebmanApiSign\Driver\ArrayDriver;
...
return [
    ...
    */
    ...
    'timeout' => 30,  //接口sign超时范围，客户端请求的timestamp和服务器时间对比出超时时间秒，0不限制
    'table' => 'app_sign', //表名
    ...
    'app_sign' => [
        [
            'app_id' => '1000000001', //应用id
            'app_name' => getenv('APP_NAME'), //应用名称
            'status' => 1,        //状态：0=禁用，1=启用
            'expired_at' => null, //过期时间，例如：2023-01-01 00:00:00，null不限制
            'app_secret' => '', //应用秘钥 不启用RSA使用
            'encrypt_body' => 0, //状态：0=禁用，1=启用 算法：aes-128-cbc 是否加密body传入加密后的报文字符串，启用RSA需要使用自动生成的app_secret进行对称加密，否则使用固定的app_secret进行对称加密
            'rsa_status' => 1, //状态：0=禁用，1=启用 启用RSA，主要用rsa加密随机生成的app_secret，而不使用固定app_secret
            /**
             * sign私钥 RS256加密
             */
            'private_key' => file_get_contents(base_path('resource/app-sign/private.key')),
            /**
             * sign公钥 RS256加密
             */
            'public_key' => file_get_contents(base_path('resource/app-sign/public.key'))
        ],
    ],
];
